# CareerCoded Journal Management Platform V2

A second monolithic MERN-style publishing system for CareerCoded with a different editorial journal interface. One Express server serves both the REST API and React frontend. Editors can create, update, publish, unpublish, and delete journal entries. Users can register, login, read published entries, search/filter content, and like posts.
...
- UI: Responsive editorial public pages, user profile, editor dashboard, entry manager

## Features

- User registration and login
- Editor/admin login with role-based authorization
- Protected API routes
- Journal entry CRUD for editors
...
Default editor/admin account:

```text
journaladmin@careercoded.com
Admin12345
```
...
- Deploy one Node.js app from this repository.
- Run `npm run install:all`, then `npm run build`, then `npm run start --prefix server`.
- The React app uses relative `/api` requests, so no separate frontend API URL is required.
- Use a hosted MongoDB database such as MongoDB Atlas for production.
